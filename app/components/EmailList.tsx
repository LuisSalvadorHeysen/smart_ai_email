...
export default function EmailList() {
    ...
    const handleEmailClick = async (id: string) => {
        ...
        try {
            ...
            // Check for internship updates
            const internshipRes = await fetch('/api/ai/internship', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ text: emailData.textBody || emailData.htmlBody })
            });

            const internshipData = await internshipRes.json();

            if (internshipData.isInternshipEmail && internshipData.company && internshipData.position) {
                setApplications(prev => {
                    const existingIndex = prev.findIndex(app => 
                                                         app.company === internshipData.company && 
                                                             app.position === internshipData.position
                                                        );

                                                        const newApplication = {
                                                            company: internshipData.company,
                                                            position: internshipData.position,
                                                            status: internshipData.status || 'Received',
                                                            date: internshipData.date || new Date().toISOString().split('T')[0],
                                                            notes: internshipData.notes || 'Automatically tracked from email'
                                                        };

                                                        const updated = [...prev];
                                                        if (existingIndex > -1) {
                                                            updated[existingIndex] = { ...updated[existingIndex], ...newApplication };
                                                        } else {
                                                            updated.push(newApplication);
                                                        }

                                                        localStorage.setItem('internshipApplications', JSON.stringify(updated));
                                                        return updated;
                });
            }
        } finally {
